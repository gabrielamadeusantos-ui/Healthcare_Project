...
def transform_data(input_buffer, file_name):
    ...
    # 2. Fix column shifting/misalignment
    def fix_shift(row):
        try:
    ...
    # 3. Clean spaces and special symbols
    regex_clean = r'[^\w\s\-\+\,]'
    ...
    # 5. Clean Doctor column (remove titles)
    ...
    # 10. Date Processing (Admission and Weekday)
    date_columns = ['Date of Admission', 'Discharge Date'] 
    
    # 10.1: Convert to actual datetime format
    for col in date_columns:
        if col in df.columns:
            df[col] = pd.to_datetime(df[col], errors='coerce', dayfirst=False)

    # 10.2: Create Admission Weekday column
    ...
    # 10.4: Format original dates to YYYY-MM-DD
    for col in date_columns:
        if col in df.columns:
